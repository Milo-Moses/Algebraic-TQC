...
\begin{defn}\label{simple-object-definition} A {\em simple object} $A$ in a fusion category $\cC$ is an object which has no direct sum decomposition into smaller objects. That is, $A\ncong B\oplus C$ for any non-zero objects $B,C\in \cC$ where $\oplus$ denotes the biproduct in $\cC$.
\end{defn}
...
\end{prop}
\begin{proof} \Note{Both maps are the same when precomposed with $\eta'$ and postcomposed with $\eta^\dagger$ for any $\eta',\eta^\dagger$ in the orthonormal basis. Since it is a basis, we can take linear combinations to conclude that they are the same precomposed and postcomposed with any map, and thus they are the same map. }
\end{proof}

\begin{cor}\label{resolution-of-id-cor} Let $\cC$ be a modular category.  For all simple objects $A_1, A_2... A_n\in \cC$, we have that 

\Note{Generalized version of proposition \ref{resolution-of-id-prop}.}
\end{cor}
\begin{proof} \Note{This is a straightforward proof by induction}.
\end{proof}
...
\begin{rem} We will discuss quantum dimensions in more detail in subsection [ref]. \Note{is there anything I need to recall now? Probably that they are non-zero, and positive when $\cC$ is unitary.}
\end{rem}
...
\begin{rem}
The question is whether or not the quantum dimensions are {\em positive} real numbers. We recall that we defined a unitarizable spherical fusion category to be one in which the quantum dimensions are all positive. It is at this point that this becomes relevant. In particular, if $\cC$ is unitarizable then its quantum dimensions are eigenvalues of $N^A$, and their corresponding eigenvector $\bold{d}$ has positive real entries.\Note{Not sure how much of this stuff I will have already said before.}
\end{rem}
...
\Note{Not sure what direction I should take for statements about unitary fusion categories like corollary \ref{unitarizable-frobenius-perron}. Should I state them for unitary fusion categories, or for unitarizable fusion categories, which have less baggage (I can define a unitarizable fusion category to be one with positive quantum dimensions, and then prove that that a fusion category admits a unitary structure if and only if it is unitarizable).}
...
We now know that the $S$ and $T$ matrices give a modular representation {\em up to phase}! We still need to work out the details of the phases. A key part is the following computation:
...
We take a moment to observe that the quantity $p_{\cC}^-/p_{\cC}^+$ is of great interest to physicists. It is related to the {\em chiral central charge} $c_{\text{top}}$ of the theory by the formula
