...
\begin{quote}
``Throughout my career I have been successful inventing toy models, some simple models that capture important features of a more complex problem." - Alexei Kitaev\footnote{[WORK: find video]}
\end{quote}
...
Here, \textit{ground state} refers to a state in the system with lowest possible energy. Note that in the previous description of spin liquids, all electrons having the same spin is a result of them being in the lowest energy state. An ``excited" electron with deviant spin will raise the energy of the system. In this way, quasiparticles can be interpreted as excitations of the topological quantum material. [WORK: This needs more explination.]This motivates the fact that (topological) quantum computers must be exceptionally cold to function: Any extra energy will correspond to extra excitations, causing the computer to malfunction, since information is only stored in ground states.
...
To describe Topological Quantum Computation (TQC), one has to describe topological quantum phases of matter. Topological Quantum Field Theories (TQFTs) are the most immediate way of doing this, as described in Section [WORK: ref]. However, there are a few problems with this method:

\begin{enumerate}
\item There is a large amount of data that needs to be specified to define a TQFT. Namely, one needs to keep track of what vector spaces are assigned to every surface, and what morphisms are assigned to every bordism. As was seen in subsection [WORK: ref], this can make defining TQFTs very difficult, and can lead to the verification of even basic axioms being difficult.
...
For those needing a refresher on Category Theory, Appendix [WORK: ref] contains the neccecary prerequisites. For this subsection, however, no knowledge of categories will be needed. This is becasue MTCs also have an elementary description as a finite collection of numbers representing physical properties of anyons.
...
The most fundamental operation on anyons is \textit{fusion}, which we have up to now seldom discussed. The reason for this is that the elementary description of the toric code given in Section [WORK: ref] works perfectly well without introducting fusion, so the notion was ommited for clarity. We now revisit the picture of Section [WORK: ref], with our eyes towards fusion.

Fusion is the process in which two elementary particles collide, and output other types of particles. [WORK: give relatable examples of fusion].
...
\item (Fusion coefficients) Natural numbers $N_{c}^{a,b}$ for all $a,b,c\in \LL$.
\end{enumerate}
...
Our current main examples can be found in Figure [WORK: make figure]. We comment additionally on the last box, representing fusion. We observe that $a\otimes b$ could be the sum of many factors, not only $c$. What this singles out is a \textit{choice} of funcion $a\otimes b\to c$, i.e., a chocie of how to fuse $a\otimes b$ together and get a $c$. In general there will be multiple ways, and the choice is mostly arbitrary. This informal notion of funcion will be put in firmer footing in subsection [WORK: ref] when we use category theory.

Consider a function $(a\otimes b)\otimes c \to d$. This can be decomposed as taking a function $a\otimes b\to n$, then $n\otimes c\to d$,

$$(a\otimes b)\otimes c\to n\otimes c\to d.$$

Given a function $a\otimes (b\otimes c)\to d$, one can decompose by taking functions $b\otimes c\to m$ and $a\otimes m\to d$. Writing out the decomposition, we get

$$a\otimes (b\otimes c)\to a\otimes m \to d.$$

These two processes can be represented in terms of each other by a linear combination. In the graphical language, this is expressed by
...
Again, the hexagon is a mess of symbols when written out explicitely. In graphical language, it becomes the fact that going either way around the following 6-term diagram gives the same answer (hence the name hexagon):
...
The spherical condition is the statement that [WORK: make spherical clear].
...
It is important to remember that even though the definition is long and complicated, at the end of the day these are still just finite collections of numbers satisfying a finite number of polynomial equations. In the case of the toric code, all of the relevant pieces of data are summarized in Figure [WORK: Make table].

\subsection{The category-theoretic viewpoint}
\label{Category viewpoint}

In this subsection, we give the category theoretic perspective on Modular Tensor Categories (MTCs). As with subsection [WORK: ref], we will build up to the definition of a MTC slowly. Thinking of MTCs as corresponding to ``non-degenerate" finite abelian groups, we will start with the category-theory versions of monoids\footnote{A monoid is a set equipped with an associative binary operation and an identity element}, groups, finite groups, finite abelian groups, and then end with non-degenerate finite abelian group.  A table giving algebraic notions and their category theoretic analogues is found in Figure [WORK: make table]. We begin with the generalization of monoid:
...
\begin{enumerate}
\item (Triangle identity) For all $x,y\in \Ccat$, we have the commutative diagram [WORK: make commutative diagram]

\item (Pentagon identity) For all $x,y,z,w\in \Ccat$, we have the commutative diagram [WORK: make commutative diagram]
\end{enumerate}
...
\begin{theorem} Let $\Ccat$ be a monoidal category. Then

\begin{enumerate}
\item (MacLane) The category $(\Ccat,\otimes_{\Ccat})$ is monoidally equivilant to a category $\Dcat$ in which $A\otimes (B\otimes C)=(A\otimes B)\otimes C$, $1\otimes A=A\otimes 1=1$, and $\alpha,\lambda,\rho$ are all the identity functors, for all $A,B,C\in \Dcat$. 
\item The category $\Ccat$ is monoidally equivilant to a category in $\Dcat$ in which all objects which are isomorphic are equal.
\end{enumerate}

However, $\Ccat$ is \textbf{not} in general equivilant to a category which is both at the same time. [WORK: Add reference. nLab has good stuff. T Hagge's thesis.]
\end{theorem}
...
\begin{definition}[Monoidal functor] [WORK: Give definition.]
...
\item (Has binary biproducts) For all $A,B\in \Ccat$ the biproduct of $A$ and $B$ exists. This biproduct is denoted $A\oplus B$, and refered to as the direct sum. Recall that a biproduct is an object [WORK: fill out definition].
\item (Has kernels and cokernels). [WORK: fill out definition]
\item (All monomorphisms and epimorphisms are normal). [WORK: fill out definition]
\end{enumerate}
\raggedleft\qedsymbol{}
\end{definition}
...
\begin{enumerate}
\item A $\CC$-linear abelian category $\Ccat$.
...
\begin{enumerate}
\item (Rigidity) [WORK: Add triangle identitites]
...
\begin{proposition} Let $\Ccat$ be a fusion category. Let $\LL$ be the set of isomorphism classes of simple objects in $\Ccat$. For any $a,b,c\in \LL$, define

$$N^{a,b}_c=\dim_{\CC}\left(\Hom(c,a\otimes b)\right).$$
...
The interesting fact is that one can \textit{not} go the other direction. Namely, we have the following:

\begin{theorem}[Ocneanu rigidity]. [WORK: find citation] Let $\LL$ be fusion system. There are finitely many (possibly $0$!) fusion categories, up to [WORK: what is the correct notion here?], which give a system [WORK: what is the correct notion here?] to $\LL$ under the process described in proposition [WORK: ref].
\end{theorem}

The correct correspondence is in fact between fusion categories and $6j$ fusion systems.

\begin{proposition}. [WORK: fusion cat to  6j system]
\end{proposition}

\begin{proposition}.[WORK: find citation]Let $\LL$ be a $6j$ fusion system. There is a unique fusion category which gives $\LL$ under the process described in proposition [WORK: ref].
\end{proposition}

[WORK: Add Yondea lemma discussion.]

We now define our analogues of abelian groups, braided fusion categories:

\begin{definition}[Braided fusion category]. [WORK: Give definition]
\end{definition}

[WORK: Show that this gives a braided $6j$ fusion system]

\begin{definition}[Modular Tensor Category]. [WORK: Give definition]
\end{definition}

[WORK: Show that this gives you a modular tensor fusion system]
