...
For example, we demonstrate that the assignment $A\mapsto \CC[A]$ is functorial.

\begin{proposition} Define the following data for a functor $\Set \to \Grp$:

\begin{enumerate}
\item Assigning $\CC[A]$ to each set $A\in \Set$.
\item Assigning the linear map
...
to $f:A\to B$, for all $A,B\in \Ccat$.
\end{enumerate}

This is a functor, which we denote $\CC[\--]$.
\end{proposition}
\begin{proof}. [WORK: do proof].
\end{proof}

This abstracted language of functors gives the following meta-fact:
...
\item (Composition) Given $F\in \Hom(\Ccat,\Dcat)$, $G\in \Hom(\Dcat,\Ecat)$, we define the composition $G\circ F: \Ccat \to \Ecat$ to be the functor sending an object $A\in \Ccat$ to $G(F(A))\in \Ecat$, and sending $f\in \Hom(A,B)$ to $G(F(f))\in \Hom(G(F(A)),G(F(B)))$. 
\end{enumerate}
...
This fundamental duality of vector spaces is something that one desires to emulate, or more accurately, the functorial embedding $V\hookrightarrow{}V^{**}$. Switching perspectives from points to functions can be illuminating. The version of duality most similar to this in category theory is known as Yoneda's lemma, where we can switch perspectives from objects to functors.

\begin{lemma} Let $\Ccat$ be a category. Define the data for a category as follows:

\begin{enumerate}
\item (Objects) $\Ccat$, but labeled with the notation $A^{\op}$ instead of $A$, to distinguish between elements of $\Ccat$ and the new category.
\item (Morphisms) Given objects $A^{\op},B^{\op}$, $A,B\in \Ccat$, we define

$$\Hom(A^{\op},B^{\op})=\Hom_{\Ccat}(B,A).$$
\item (Composition) Given morphisms $f^{\op}:A^{\op}\to B^{\op}$, $g^{\op}:B^{\op}\to C^{\op}$, $A,B,C\in \Ccat$, we define

$$g^{\op}\circ f^{\op}=(f\circ g)^{\op}.$$
\end{enumerate}

This data gives a well defined category, which we call $\Cat^{\op}$.
\end{lemma}
...
\begin{align*}
\Ccat &\xrightarrow{} \bold{Hom}(\bold{Hom}(\Ccat^{\op},\Set),\Set).\\
...
\begin{align*}
\Ccat \times \bold{Hom}(\Ccat^{\op},\Set)&\to\Set\\
(A,F)&\mapsto F(A)
\end{align*}
