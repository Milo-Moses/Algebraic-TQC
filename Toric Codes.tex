...
One key physical observation [WORK: I got this wrong. The probabilities have a normalizing part coming from the quantum dimension. The paper to reference here is Kitaev and Preskill's TEE] is that when two quasiparticles are fused, the probability of observing any other quasiparticle will always be \textit{rational}. Clearing denominators, we can thus represent the fusion operation as assining an integer ``multiplicity" to every anyon type, which gives the relative probability of that particle being observed. For notation, we will say that if $a$ and $b$ are quasiparticle types which fuse to each anyon type $c$ with relative probability $N^{a,b}_{c}$, then
...
\begin{proposition} Let $G$ be a finite group, and choose $V\in \Vecc_G$. There is a canonical bijection
...
\begin{proposition}\label{explicit-trace} Let $f:A\to A$ be an endomorphism in a fusion category $\Ccat$. Fix a decomposition $A\cong \bigoplus_{i\in I}A_i$ of $A$ into simple objects $A_i$. Moreover, we take the decomposition such that if $A_i\cong A_j$ then $A_i=A_j$. We can decompose
...
\item This follows from Proposition \ref{explicit-trace}.
...
\item .[WORK: Abelian $\implies$ deterministic fusing]
\end{enumerate}
