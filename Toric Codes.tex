...
Additionally, a fusion system is required to satisfy the following properties: [WORK: Do I need to include ``Frobenius Reciprocity? Sebastien Palcoux does on his webpage."]
...
[WORK: I feel like I'm cheating by assuming all pivotal coefficients are $1$. Is this a bad thing to do? Is there a twist-centric version?]
...
\begin{theorem} Let $A\in \Ccat$ be a simple object in a MTC. The following claims about the quantum dimension are all true:

\begin{enumerate}
\item .[WORK: state the more basic things which apply in general. This first one should be $d_A\neq 0$. Do I get any other restrictions on where $d_A$ can be?]

\item .[WORK: The matrix $N_A$ may not have a unique largest eigenvalue. Zhenghan said that $d_A$ would be a Galois conjugate of one of the largest roots, or something...]
\end{enumerate}

If $\Ccat$ is unitary, the we moreover have the following:

\begin{enumerate}
\item ([WORK: get reference]) If $1\leq d_A\leq 2$, then $d_A=2\cos(\pi /m)$ for some integer $m\geq 3$. Otherwise, $d_A\geq 4$.

\item $d_A=1$ if and only if $A$ is an abelian anyon. That is, if and only if [WORK: there's a few things I could put here. I could put "braiding gives scalar multiples of identity" and "deterministic fusing of partices" or both or neither...]
...
\end{enumerate}
\end{theorem}
\begin{proof}. [WORK: prove what I can]
\end{proof}
...
\begin{theorem}[\cite{bakalov2001lectures}] Let $\Ccat$ be an MTC, with set $\LL$ of isomorphism classes of simple. We define the $S$-matrix of an MTC to be the matrix whose $(a,b)$ coefficient is
...
finishing the proof. Note that this inequality can be tight, such as in the case of the toric code.
...
We now move on to point (4). Taking the distinguished morphisms $i_{A^*}=(p_{A})^*$, $i_{B^*}=(p_B)^*$, $p_{A^*}=(i_A)^*$, $p_{B^*}=(i_B)^*$, the fact that the functor $(\--)^*$ is linear clearly implies that $(A\oplus B)^*\cong A^*\oplus B^*$. Explicitely we can describe evaluation on $A\oplus B$ by
...
where the first isomorphism comes from Exercise \thesection.7, the downwards arrow is the identity on $A\otimes A^*$, $B\otimes B^*$ and zero on the other factors, the map to $1\oplus 1$ is $\ev_A\oplus \ev_B$, and the last arrow is the sum of the two projection maps $1\oplus 1\to 1$. Coevaluation can be described similarly.
...
Unpacking the definition, we compute
...
\begin{proposition} Let $\Ccat$ be a ribbon fusion category, with set $\LL$ of isomorphism classes of simple objects.

\begin{enumerate}[(a)]
\item If $\Ccat$ is an MTC, the collumns of the $S$-matrix are simultaneous eigenvectors for the fusion matricies

$$N^{a}=(N^{a,b}_{c})_{b,c\in \LL}.$$

The eigenvalues are quantum dimensions [WORK: find correct statement]
...
\end{enumerate}
\end{proposition}
\begin{proof} .[WORK: do proof. Part (a) is postponed to the exercises]
\end{proof}
...
\item Let $\Ccat$ be a $\CC$-linear abelian monoidal category, whose tensor product, associativity, and unitors are all linear functors.  Show that
...
\item The tensor product is right exact. That is, for every short exact sequence $0\to A\to B\to C\to 0$ in $\Ccat$, the sequence [WORK: it might be exact on the left too? Moreover, we might need this left exactness to deduce the ring structure...]
...
It is interesting to note that in some important cases these properties uniquely specify the tensor product. In particular, suppose we have a right exact functor which preserves direct sums between categories of modules over a right. The Eilenberg-Watts theorem says that this functor will be naturally isomorphic to tensoring with some bimodule This theorem was proved simultaneously by Eilenberg \cite{eilenberg1960abstract} and Watts \cite{watts1960intrinsic}, and has since been generalized to the setting of fusion categories \cite{femic2016eilenberg}.

\item Let $K(\Ccat)$ be the abelian group generated by objects in $\Ccat$, subject to the relation that $[A]+[C]=[B]$ whenever there is an exact sequence

$$0\to A\to B\to C\to0.$$

We call this the \textit{Grothendieck group} of $\Ccat$. It is an important invariant, and serves as the basis of algebraic K-theory \cite{quillen2006higher}. Exercise \thesection.5 tells us that the product $[A]\cdot [B]=[A\otimes B]$ gives $K(\Ccat)$ a ring structure, and hence we get a \textit{Grothendieck ring}. For MTCs, we can compute with this ring very explicitely, as shown in this exercise.

\begin{enumerate}[(a)]
\item Let $\Ccat$ be a ribbon fusion category. Let $\LL$ denote the set of isomorphism classes of simple objects of $\Ccat$. Define a map $\mu: K(\Ccat)\otimes_{\ZZ}\CC\to \CC[\LL]$ by sending $[A]$, $A\in \Ccat$, to

\begin{equation*}
\tikzfig{grothendieck-ring-iso}
\end{equation*}

where $B$ is a representative for $b$ as usual. Show that this induces a well defined morphism of $\CC$-algebras.

\item Suppose further that $\Ccat$ is an MTC. Show that

$$\mu([a])=\sum_{b\in \LL}\frac{s_{a,b}}{d_b}\left|b\right>.$$

Conclude from the invertability of the $S$-matrix (Proposition [WORK: ref]) that $\mu$ induces an isomorphism $K(\Ccat)\otimes_{\ZZ}\CC\cong \CC[\LL]$. Note the key use of Proposition [WORK: ref], which says that the quantum dimensions are non-zero.

\item Comparing that the $S$-matrix simultaneously diagonalizes the fusion matricies [WORK: I don't know what the exact correct statement is. Also, I don't know whether there should be a hint here. I should be able to do it on my own...]
\end{enumerate}

\item This exercise builds up the basic theory of Deligne tensor products, as first introduced in \cite{deligne2007categories}. The Deligne tensor product does not exist between every pair of abelian categories - there are countexamples, based on an eariler work of Soublin \cite{soublin1970anneaux}. For such generality one must appeal to the Kelly tensor product \cite{franco2013tensor}. We thus restrict ourselves to the case that $\Ccat$ is fusion for simplicity.
...
commute. It is a theorem of Deligne that the Deligne tensor product exists. Show that it is unique up to unique isomorphism. Recall that a functor $F$ of abelian categories is \textit{right exact} if for every short exact sequence $0\to A\to B\to C\to 0$ there is a short exact sequence

$$F(A)\to F(B)\to F(C)\to 0.$$

We say that a functor $F$ from $\Ccat\times \Dcat$ is right exact if for every short exact sequence $0\to A\to B\to C\to 0$ in $\Ccat$ and $D\in \Dcat$ the sequence
...
\item Show that for finite groups $G,H$, we have that $\Vecc_G\boxtimes \Vecc_H\cong \Vecc_{G\times H}$. Note that this is part of a more general phenominon, that the Deligne tensor product of representation categories of algebras is the representation category is the tensor product of algebras \cite[Prop. 1.11.2]{etingof2016tensor}.
...
This additionally gives an alternate characterization of MTCs. Namely, a ribbon fusion category $\Ccat$ is non-degenerate if and only if $\Ccat\boxtimes \Ccat^{rev}\to \Zcal(\Ccat)$ is an equivalence of categories \cite[8.20.12]{etingof2016tensor}.

\item Let $G$ be a finite group, and consider $\Vecc_G$ as an abelian category. Define the structures of tensor product and unitors as usual. Let $\alpha: (\-- \otimes \--)\otimes \--\to \--\otimes(\--\otimes\--)$ be a choice of associativity which turns $\Vecc_G$ into a monoidal category. Define a map $\omega_\alpha: G\times G\times G\to \CC^\times$ sending $(g_0,g_1,g_2)$ to the unique scalar $\lambda$ such that

$$\alpha_{V_{g_0},V_{g_1},V_{g_2}}=\lambda \cdot \id_{V_{g_0}\otimes V_{g_1}\otimes V_{g_2}},$$

where we use $\id$ in an abuse of notation to mean the standard associativity map, and we know that such a $\lambda$ exists since $V_{g_0}\otimes V_{g_1}\otimes V_{g_2}\cong V_{g_0g_1g_2}$ is a simple object. Show that $\omega_{\alpha}$ a $3$-cocycle. That is, it satisfes the condition

$$\omega(g_0,g_1,g_2)\omega(g_1,g_2,g_3)=\omega(g_0g_1,g_2,g_3)\omega(g_0,g_1g_2,g_3)^{-1}\omega(g_0,g_1,g_2g_3)$$

Moreover, show that this induces a bijection between the set of associativity structures on $\Vecc_G$ and the set $Z^3(G,\CC^\times)$ of $3$-cocycles on $\Vecc_G$. In fact, show that this descends to a bijection
...
We denote the monoidal category attached to a cocycle $\omega\in Z^3(G,\CC^\times)$ by $\Vecc_G^\omega$. Observe that $H^3(G,\CC^\times)$ is finite, as predicted by Ocneanu rigidity.
