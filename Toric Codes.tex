...
[WORK: A big confused about the structure vs. property thing here. Shouldn't we have twists as part of the data?]
...
In light of this proposition, we will often assume that $(A\otimes B)^*=B^*\otimes A^*$. While technically incorrect, these two objects are always canonically isomorphic and we do not lose any generality by imposing this condition. We now contrast fusion categories and fusion systems. The first claim is as follows:
...
As usual, a braided monoidal equivalence of two categories means a pair of braided monoidal functors, such that both compositions are monoidally equivalent to the identity. We now establish a useful graphical lemma about braided monoidal categories which will be useful:
...
We now move on to MTCs. A \textit{non-degenerate} braided fusion category is one in which for all simple objects $A\in \Ccat$,

$$\beta_{B,A}\circ \beta_{A,B}=\id_{A\otimes B}\,\,\forall \,\,B \text{ simple}\implies A\cong 1.$$

The physical motivation for this non-degeneracy condition is given in section \ref{Topological Quantum Computing}. It is a key subtelty of the theory that MTCs are \textit{not} simply non-degenerate braided fusion categories. Every MTC will additionally have a ``twisting" operation. The reason for this is as follows. Not only should our theory have distinguihsed moprhisms $A\otimes A^*\to 1$ and $1\to A^*\otimes A$, but it should also have distinguished morphisms $A^*\otimes A\to 1$ and $1\to A\otimes A^*$. Doing this naively using braiding works to a limited degree, but it ends up being incompatible with tensor products. Hence, we must introduce a twist to remedy the situation:
...
The key point which would be invalid if we did not add the twist is as follows\footnote{It is for this reason that we call ribbon categories \textit{ribbon}. We can replace single strands properly with their two-strand counterparts, forming bands which look like ribbons}: [WORK: use term ``topological spin" to motivate why we are using a twist-centric definition]

\begin{proposition}\label{ribbon-property} .[WORK: move this to the end of us building up the ribbon?] Let $\Ccat$ be a ribbon category. For all $A,B\in \Ccat$,
...
\end{proposition}
\begin{proof} We prove only the first identity, since the proof of the second is identical. We compute as follows:
...
Morphisms between ribbon categories should respect the twisting operation. A \textit{ribbon functor}\footnote{This is more popularly refered to as a \textit{braided tensor functor}. [WORK: Do I need to define this at all?]} between two MTCs $(\Ccat,\theta_\Ccat),(\Dcat,\theta_{\Dcat})$ is a braided monoidal functor $F:\Ccat\to \Dcat$ such that $F(\theta_{\Ccat;A})=\theta_{\Dcat;A}$ for all $A\in \Ccat$. A ribbon equivalence is thus a pair of ribbon functors whose compositions in either direction are monoidally naturall equivalent to the identity. This allows us to state our final equivalence:
...
The distinguished morphisms of a ribbon category give good ``two-sided" duals, in the sense that if $A$ is a dual of $A^*$ then $A^*$ is also a dual of $A$. That is, we can identify $A$ with $A^**$. We will later find ourselves interested more generally in the categories wit such two-sieded duals, known as pivotal categories \cite{selinger2011survey}. Formally, a pivotal category is a monoidal category $\Ccat$ paired with a monoidal natural isomorphism $(\--)\to (\--)^{**}$. Pivotal categories can be described explicitely in graphical language:

\begin{proposition}. [WORK: write out what the axioms of a pivotal category imply. Remove the associated exercise.]
\end{proposition}

[WORK: Re-do this introduction sentence]The following shows that all MTCs are pivotal, but also that braided monoidal categoires without twist are pivotal:

\begin{proposition}\label{pivotal-proposition} Let $\Ccat$ be a braided monoidal category, and let $\theta:\id_{\Ccat}\to\id_{\Ccat}$ be a natural transformation such that $(\theta_A)^*=\theta_{A^*}$ for all $A\in \Ccat$. The distinguished morphisms. [WORK: Re-do this. Go through Deligne's lemma that giving a pivotal structure is the \textit{same} as giving a twist, for braided categories. Original reference]

\begin{equation*}
\tikzfig{co-dual}
\end{equation*}

give $\Ccat$ the structure of a pivotal category.
\end{proposition}
\begin{proof}. [WORK: Do proof]
\end{proof}

The power of pivotal categories is that one can define the \textit{trace} of a morphism $f:A \to A$. The trace is a powerful operation, since it allows you to qualify morphisms in terms of dnumbers, which can be used both for linearization and defining invariants. Naturally, one wants to define trace by ``closing the loop" on $f$. That is,
...
\begin{proposition} Let $(\Ccat,\theta)$ be as in Proposition \ref{pivotal-proposition}. $\Ccat$ is a spherical category. [WORK: There is an extra condition needed which is equivalent to being spherical - cite it exactly. Lemma 4.27 in reference.]
\end{proposition}
\begin{proof}. [WORK: Do proof]
\end{proof}
...
[WORK: I want to know what's true. I DON'T think you can move $f$ to $f^*$ on the other. I can't figure out how to prove that trace commutes with composition.]
...
For compatibility with the tensor product, one needs the ribbon axiom:

\begin{proposition} Let $\Ccat$ be a ribbon category. For all $A\in \Ccat$, $f\in \End(A)$,  $\tr(f\otimes g)=\tr(f)\cdot \tr(g)$.
\end{proposition}
\begin{proof} Using the ribbon property of Proposition \ref{ribbon-property}, we compute
...
\item (Rigidity) Diagrams commuting in $\Ccat$ implies they commute in $\Zcal(\Ccat)$, hence rigidity is immediate. However, one does need to make sure that $\beta_{A^*,\--}$ is a valid half-braiding. Graphically, $\beta_{A^*,B}$ is given the compostion
...
\begin{proposition}\label{spherical-center-modular} Let $\Ccat$ be a spherical fusion category. Define the following data for a ribbon fusion category:
