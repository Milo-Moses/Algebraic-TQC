...
Of the many approaches to quantum computation, topological quantum computation (TQC) has the distinction of being both one of the most mathematically complicated and potentially powerful methods.
...
The intuition is as follows. The $\#$1 challenge in quantum computation is error correction. While quantum computers that fix errors faster than they arise (known as ``fault tolerant") can provably exist, the error rate must be unrealistically low \cite{gottesman1998theory}. \textit{Topology} is the mathematical study of those properties of geometric objects that are invariant under small perturbations. The key insight of TQC is that instead of storing quantum information in the states of individual particles, the information can be stored in topological invariants of geometric objects. As such, even when physical errors happen (e.g. the geometric object is perturbed) the information stored in the qubits remains the same. More than being error correcting, TQC is naturally error \textit{resistant}!
...
The above situation is more than just a thought experiment: this is exactly the scheme South American Incas used over 4000 years ago! They stored all sorts of information in \textit{Quipus}, intricately knotted collections of fibered strings \cite{ascher1981code} as seen in Figure \ref{fig:quipu}. Storing information in knot invariants was also common practice in ancient Chinese, Tibetan, and Polynesian cultures \cite{day2021quipus}. In a sense, these are the earliest examples of topological computation.
...
We find it illustrative here to make an analogy with classical computing. Consider the following puzzle: classical bits are stored in the magnetization of small regions of a hard disk. The magnetization of each atom is highly sensitive to thermal fluctuations. So why is it that classical computers seem so resistant to errors? The answer is that since all of the atoms are magnetized in the same direction, any one atom flipping will automatically be corrected back by the normalizing influence of the atoms around it: magnets are naturally error resistant. It is exactly the same with these spin liquids that quantize together. Any one electron's spin decohering will immediately be corrected by the normalizing influence of its neighbors.

In the lab, all one needs to do to make all spins in a quantum materal align is wait long enough for the system to form an equilibrium in a low energy environment. The spin conflicts between neighbors will be sorted out naturally. Any ``excited" electron with deviant spin will raise the energy of the system. Interestingly, such exited electrons behave freely like particles in the quantum material. Hence, we refer to them as quasiparticles. This motivates the fact that topological quantum computers must be exceptionally cold to function: any extra energy will correspond to extra excitations, which will interfere with computation. Compactly, we use the term \textit{ground state} to refer to a state in a system with lowest possible energy. Ground states are exactly those with no quasiparticles.

[WORK: add a paragraph here outlining particle/antiparticle pairs and braiding, to make sense of why these have to be computations.]
...
This is known as the (complexified) \textit{fusion ring} or \textit{Grothendieck ring} of $\Ccat$.

\begin{enumerate}[(a)]
\item Show that $K_{\CC}(\Ccat)$ satisfies the axioms of a $\CC$-algebra.
\item Suppose further that $\Ccat$ is a ribbon fusion category. Let $\CC[\LL]^{\func}$ denote the $\CC$-algebra on $\CC[\LL]$ whose multiplication is performed componentwise. Define a map $\mu: K_{\CC}(\Ccat)\to \CC[\LL]$ by sending $\left| a\right>$, $a\in \LL$, to 

\begin{equation*}
\tikzfig{grothendieck-ring-iso}
\end{equation*}

where $A,B$ is a representatives for $a,b$ as usual. Show that this induces a well defined morphism of $\CC$-algebras.

\item Show that

$$\mu([a])=\sum_{b\in \LL}\frac{s_{a,b}}{d_b}\left|b\right>.$$

In the case that $\Ccat$ is an MTC, conclude from the invertability of the $S$-matrix (Proposition \ref{MTC facts} (a)) that $\mu$ induces an isomorphism $K_{\CC}(\Ccat)\cong \CC[\LL]^{\func}$. Note the key use of Proposition \ref{quantum dimensions} (a) which says that the quantum dimensions are non-zero.

\item Suppose $\Ccat$ is an MTC. Show that

$$\left| a \right>\cdot \mu^{-1}(\left|b\right>)=\frac{S_{a,b}}{d_b}\cdot \mu^{-1}(\left| b\right>).$$

Conclude that $\mu^{-1}(\left| b\right>)$ is an eigenvector for the fusion matrix $N^a$, with eigenvalue $S_{a,b}/d_b$. Use this to show that

$$S N^a S^{-1}=D^a$$

where $D^a$ is the diagonal matrix whose $(b,b)$ entry is $S_{a,b}/d_b$, proving Proposition \ref{verlinde} (c).
