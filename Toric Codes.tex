...
The logical dependence of the sections is as follows:


\begin{equation*}
  \tikzfig{dependency-tree}
\end{equation*}
...
For example, we demonstrate here that $\CC[S]$ is functorial.

\begin{proposition} Define the functor $\CC[\--]$ [WORK: give good definition, state it is functorial].
\end{proposition}
\begin{proof}. [WORK: do proof].
\end{proof}

This abstraction of functors gives the following meta-fact:

\begin{proposition} . [WORK: Define the category of categories]
\end{proposition}
\begin{proof}. [WORK: do proof].
\end{proof}
...
\begin{definition}[Natural transformation]. [WORK: define natural transformation]
\end{definition}

\begin{proposition} Let $\Ccat,\Dcat$ be categories. [WORK: define the category $\bold{Hom}(\Ccat,\Dcat)$ of functors.]
\end{proposition}
\begin{proof}. [WORK: do proof]
\end{proof}

With this, we can succinctly state the notion of equivalence we will use throughout this text. Two categories $\Ccat$ and $\Dcat$ are equivalent if there is are functors $F:\Ccat\to \Dcat$ and $F^{-1}:\Dcat\to \Ccat$ such that $F\circ F^{-1}$ and $F^{-1}\circ F$ are both isomorphic (i.e., there exists invertible natural transformations) to the identity.
...
This can be seen as saying that points are functions on functions. It is an important fact that this map is an isomorphism, and the proof relies keely on the fact that $V$ is finite dimensional - it could fail in general, where we only have an embedding $V\hookrightarrow{} V^{**}$. Formally, we have the following:

\begin{proposition}. [WORK: natural transformation between functors $\id_{\Vecc}$ and $(\--)^{**}$.]
\end{proposition}
\begin{proof}. [WORK: do proof].
\end{proof}

This fundamental duality of vector spaces is something that one desires to emulate. Switching perspectives from points to functions can be illuminating. The version of duality most similar to this in category theory is known as Yoneda's lemma, where we can switch perspectives from objects to functors.

\begin{theorem}[Yoneda's Lemma]. [WORK: state the Yoneda lemma]
\end{theorem}
\begin{proof}. [WORK: do proof]
\end{proof}

The Yoneda lemma is considered one of the most fundemental results in category theory. A philosophical description of the Yoneda lemma is as follows:

[WORK: Get Ravi Vakil quote back.]

This loose analogy become almost exactly true in the study of Topological Quantum Computing. One considers categories in which the objects correspond to collections of quasiparticles, and morphisms correspond to physical transformations of the system. Categories that arrise in this fashion are known as Modular Tensor Categories. Modular Tensor Categories are \textit{monoidal} categories, which means that there is a tensor product $A\otimes B$ of objects. This corresponds to putting two particles together. Functions $A\otimes B\to C$ correspond fusion of $A$ and $B$ into $C$. The \textit{braiding} of two particles is an isomorphism between $A\otimes B\to B\otimes A$. Topological Quantum Computing is based on the fact that $A\otimes B$ and $B\otimes A$ are isomorphic, but non-canonically, meaning that braiding different ways can produce different results.

When storing numberical data about braiding, one does not recording the braiding isomorphim $A\otimes B\to B\otimes A$ directly. Instead, one stores an isomorphism between $\Hom(A\otimes B, C)$ and $\Hom(B\otimes A, C)$ for each $C$. That is, one relates how $A\otimes B$ and $B\otimes A$ fuse relative to other particles. This is practically a much nicer thing to do. This gives a natural isomorphim between $\Hom(A\otimes B,\--)$ and $\Hom(B\otimes A, \--)$, and thus Yoneda's lemma tells us this induces an isomorphism $A\otimes B\to B\otimes A$. This is a routine practice in our study of Modular Tensor Categories, and the Yoneda-perspective is often assumed implicitely.

The Yoneda lemma can be stated in a way more reminiscent of vector space duality. There is a natural map $V\times V^*=V\times \Hom(V,\CC)\to \CC$, sending $(v,\varphi)$ to $\varphi(v)$. Vector space duality is exactly the fact that this pairing is non-degenerate, in the sense that if $\varphi(v)=0$ for all $v$ then $\varphi=0$, and if $\varphi(v)=0$ for all $\varphi$ then $v=0$. There is a pairing on the level of categories,$\Ccat \times \left[\Ccat^{\text{op}},\Set\right]\to \Set$, and the Yoneda lemma is exactly the statement that it is non-degenerate. Observe that to make this statement formal, we will have to define the product $\times$ of categories. In some sense, we already have: $\times$ is the product in the category of categories! Explicitely, we can also describe the product as follows:

\begin{proposition}. [WORK: give explicit description of product in category of categories.]
\end{proposition}
\begin{proof}. [WORK: do proof]
\end{proof}

We now give our formal restatement of Yoneda's lemma:

\begin{proposition}. [WORK: give 2nd statement of Yoneda lemma]
\end{proposition}
\begin{proof}. [WORK: do proof]
\end{proof}
...
[WORK: Define relevant equivalence in sections, properly]
