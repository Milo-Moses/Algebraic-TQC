...
Additionally, a rigid category is required to satisfy the following property that $(\ev_A \otimes \id_A)\circ (\id_A\otimes \coev_A)=\id_A$ and $(\id_{A^*}\otimes \ev_A)\circ (\coev_{A}\otimes \id_{A^*})=\id_{A^*}$ for all $A\in \Ccat$. 
...
\item $\theta_{A^*}=\left(\theta_A\right)^*$ for all $A\in \Ccat$. [WORK: we have yet to define the dual of a function. Maybe pull this up a bit?]
\end{enumerate}
\raggedleft\qedsymbol{}
\end{definition} 
...
This condition makes the second hexagon identity tautological. Similarly, the definition of tensor product given is not strictly correct - one must add the correct associator terms, making the first hexagon identity tautological. Lastly one must verify the half-braidings defined on the tensor unit/tensor product are actually half braidings, i.e., that they satisfy the hexagon condition. These follow from straightforward computations, which we leave as exercises. This completes the proof.
...
\begin{proposition}[\cite{muger2003subfactors}]\label{spherical-center-modular} Let $\Ccat$ be a spherical fusion category. Define the following data for a ribbon fusion category:
...
\begin{proposition} Let $A\in \Ccat$ be an object in a fusion category. There is a non-canonical isomorphism $A\cong A^{**}.$
\end{proposition}
...
\begin{proposition} Let $(\Ccat,i)$ be a pivotal category. We graphically define the distinguished morphisms
...
\end{proposition}
\begin{proof} We begin with the forward direction, proving that both properties are satisfied.
...
We now move on to the converse direction. [WORK: do proof]
...
In the case of braided monoidal categories, it is an amazing fact that pivotal structures can be alternatively described in terms of \textit{twists}:
...
When working with pivotal categories, one only has the weaker notions of ``left trace" and ``right trace". Abstractly, we can think of spherical categories as the minimal structure in which traces are well defined. The spherical property is automatic for braided pivotal categories:

\begin{proposition} All braided pivotal categories are spherical.
\end{proposition}
\begin{proof}. [WORK: Do proof]
\end{proof}

For $\Vecc_G$, we have the following:

\begin{proposition} The double dual isomorphism on vector spaces induces a pivotal structure on $\Vecc_G$.
\end{proposition}
\begin{proof}. [WORK: do proof]
\end{proof}


We now express ribbon categories in twist-theoretic language, as promised:
...
holds [WORK: literally rubbish - assumes we have a twist. Re-do this in graphical language.].
...
In particular, ribbon categories can be equivalently described as braided pivotal categories for which the formula (1) holds for every $A$.

\end{proposition}
\begin{proof} [WORK: do the proof]
\end{proof}

We now demonstrate some basic facts traces, after a lemma:
...
If $\Ccat$ is moreover a ribbon fusion category, then $\tr(f\otimes g)=\tr(f)\cdot \tr(g)$.
