...
[WORK: this is a cop out. One really needs to know this makes a functor, and $f^{**}=f$. Work it into the next proposition.]Additionally, we observe that given any $f:A\to B$ in a fusion category, there is a canonical map $f^{*}:B^*\to A^*$ defined by the composition
...
The most important fact about duals is that they are unique, in the sense that any two choices of duals will be canonically isomorphic:

\begin{proposition} Let $A\in \Ccat$ be an object in a fusion category. Suppose $(\tilde{A}^{*},\tilde{\ev}_A,\tilde{\coev}_A)$ is another choice of dual for $A$, that is, the triple satisfiying the axiom of rigidity. There is a canonical isomorphism $A^{*}\xrightarrow{\sim}\tilde{A}^{*}$ making the diagrams
...
\begin{enumerate}
\item We wish to show that the diagram

\[\begin{tikzcd}
	& {(VW)(VW)^*(VW)^{**}} & {(VW)^*(VW)(VW)^{**}} & {(VW)^{**}} & VW \\
	VW &&&& WV \\
	& {(VV^*V^{**})(WW^*W^{**})} & {(V^*VV^{**})(W^*WW^{**})} & {V^{**}W^{**}} & VW
	\arrow["{\beta_{VW,(VW)^*}}", from=1-2, to=1-3]
	\arrow["{\beta_{V,V^*}\beta_{W,W^*}}"', from=3-2, to=3-3]
	\arrow["{\ev_{V}\ev_{W}}"', from=3-3, to=3-4]
	\arrow["{\ev_{VW}}", from=1-3, to=1-4]
	\arrow[from=1-4, to=1-5]
	\arrow[from=3-4, to=3-5]
	\arrow["{\beta_{V,W}}"', from=3-5, to=2-5]
	\arrow["{\beta_{W,V}}"', from=2-5, to=1-5]
	\arrow["{\coev_{(VW)^*}}"{description}, from=2-1, to=1-2]
	\arrow["{\coev_{V^*}\coev_{W^*}}"{description}, from=2-1, to=3-2]
\end{tikzcd}\]

commutes. [WORK: This seems annoying.]
