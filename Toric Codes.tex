...
in $\Hom(A^{*},B^{*})$ is a monoidal functor. Moreover, this functor induces a monoidal equivalence of between $\Ccat^{\op}$ and $\Ccat$.
\end{enumerate}
\end{proposition}
...
as desired. The fact that $(\id_{A})^{*}=\id_{A^{*}}$ follows immediately from rigidity. [WORK: there are other axioms (probably trivial) to make this a monoidal functor]. We now show that $(\--)^{*}$ full, faithful, and essentially surjective, which by Exercise \thesection.6 is enough to conclude that $(\--)^{*}$ induces an equialence of categories. For fully faithfulness, we define a linear mpa $\Hom(A^{*},B^{*})\to \Hom(B,A)$ taking the morphism $f:A^{*}\to B^{*}$ to the composition
...
\item (Inverse/anti-particle) This property comes from the fact firstly that duals of simple objects are simple, and secondly that they are unique. Uniquenesss is Proposition \ref{rigidity}. To show that duals of simple objects are simple, we note that given any object $A\in \Ccat$ and any morphism $f:A^{*}\to A^{*}$, we can define a morphism $A\to A$ by the formula [WORK: make diagram].

Applying rigidity, we see that this serves as an inverse to the map $\End(A)\to \End(A^{*})$ sending $f$ to $f^{*}$. Hence, we get an isomorphism between these two hom spaces. Thus, the endomorphism space of an object and its dual are always of the same dimension. In particular, the dual of a simple object is simple as desired.
...
The interesting fact is that one can \textit{not} uniquely go the other direction. That is, there there are non-equivalent fusion categories which give the same fusion system. To define the correct notion of equivalence, we need to know what natural transformations are allowed between monoidal functors:

\begin{definition}[Monoidal natural transformation]. [WORK: hey]
\end{definition}

A \textit{monoidal equivalence} of categories is a pair of monoidal functors between them, such that both compositions are monoidally naturally equivalent to the identity. We have the following:
...
The space $\End(1)$ is one dimensional by Schur's Lemma (Theorem \ref{Schurs Lemma}), with generator $\id_1$. Hence, we can canonically identify $\tr(f)$ with a complex number. Graphically, we represent trace as the closed loop [WORK: BRAID make diagram, fix notation for un-standard cup and cap using twist to make this rigorous.].
...
\item .[WORK: BRAID 2nd formula for trace, putting $\beta$ and $\theta$ before $f$]
